...
const Obaveze = ({ obaveza, promjenaIzvrsenosti, brisiObavezu }) => {
  const oznakaZaIzvrsenost = obaveza.izvrseno ? "Izvrseno" : "Nije izvrseno";

  const oznakaZaVaznost = obaveza.vazno ? "Vazno" : "Nije vazno";
  const datum=obaveza.datum.slice(0,15)
  ...
  if (obaveza.izvrseno === false) {
    return (
      <tr className="obaveza">
        ...
        <td>{datum}</td>
        <td>
          <button id="izv" onClick={promjenaIzvrsenosti}>
            {oznakaZaIzvrsenost}
          </button>{" "}
        </td>
        <td>
          <button id="izv" onClick={brisiObavezu}>
